...
Avant de suivre cette procédure, vous devez connaître link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["Volumes persistants"]et savoir si vous souhaitez les utiliser. Les volumes persistants sont facultatifs mais recommandés pour la restauration des données de configuration du nœud de gestion en cas de perte de machine virtuelle.
...
. Téléchargez la certification OVA ou ISO pour votre installation depuis la link:https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["Logiciel Element"^] page du site de support NetApp.
+
...
NOTE: Ne modifiez pas le NTP après avoir terminé la configuration de synchronisation de l'heure, car il affecte le NTP lorsque vous exécutez link:task_mnode_install.html#set-up-the-management-node["commande setup"]sur le nœud de gestion.
...
*** *--Storage_mvip (-sm) [adresse MVIP]* : adresse IP virtuelle de gestion (MVIP) du cluster de stockage exécutant le logiciel Element. Configurez le nœud de gestion avec le même cluster de stockage que celui que vous avez utilisé pendant link:task_mnode_install.html#configure-time-sync["Configuration de serveurs NTP"].
...
.. Exécutez `setup-mnode` la commande.
...
.. A partir du corps de réponse code 200, copiez et enregistrez le pour l'installation et l' `id`utiliser ultérieurement.
+
